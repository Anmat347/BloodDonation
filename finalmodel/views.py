    ...
    if request.method=='POST':
        if request.POST.get('uname') and request.POST.get('pwd'):
            ...
            with connection.cursor() as cursor:
                ...
                #print(row)
                if len(row)==1:
                    dnr = tblblooddonars.objects.all().count()
                    ...
                    print(actdnr)
                    admdict = {'dnr':dnr, 'msg':msg,'acc':acc,'user':user,'actdnr':actdnr}
...
def dummypredict(request,did):
    ...
    with connection.cursor() as cursor:
        ...
        ans=kNN.predict([li])
        #print(ans)
        quer1 = "select * from finalmodel_signupuser where uid = %s" 
        cursor.execute(quer1,[userid])
        row = cursor.fetchone()
        print(row)
        name = row[1]
        email = row[2]
